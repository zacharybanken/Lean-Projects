...
-- from chatGPT (test)
theorem ceil_div_mul_eq_ceil_div_ceil {x : ℝ} {m n : ℕ} (hmn : 0 < m ∧ 0 < n) :
  int.ceil (x / (↑ m * ↑ n)) = int.ceil (int.ceil (x / ↑ m) / ↑ n) :=
begin
  have hmn' : (0 : ℝ) < (m : ℝ)  * (n : ℝ)  := by library_search,
  have h1 : x / (m.to_real * n.to_real) ≤ int.ceil (x / m.to_real) / n.to_real,
  { rw div_le_div_right hmn.2.to_real,
    apply int.floor_le },
  have h2 : int.ceil (x / m.to_real) / n.to_real ≤ int.ceil (x / (m.to_real * n.to_real)),
  { rw div_le_div_right hmn',
    apply int.le_ceil,
    rw div_nonneg_iff,
    exact le_of_lt hmn.1,
    rw mul_comm,
    exact mul_pos hmn.1.to_real hmn.2.to_real },
  apply le_antisymm,
  { apply int.le_ceil.2 h1 },
  { apply int.ceil_le.2 h2 }
end
